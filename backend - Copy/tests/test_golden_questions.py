...
GOLDEN_QUESTIONS: list[dict[str, str]] = [
    # Finanza / Margini
    {"id": "F1", "category": "finanza", "question": "Qual e il revenue with tax totale nel 2014?"},
    {"id": "F2", "category": "finanza", "question": "Mostra il margine per venditore nel 2014"},
    {"id": "F3", "category": "finanza", "question": "Top 3 venditori per fatturato nel 2014"},
    {"id": "F4", "category": "finanza", "question": "Qual e il cliente che ha speso di piu?"},
    # Logistica / Consegne
    {"id": "L1", "category": "logistica", "question": "Quanti ordini abbiamo nel 2014?"},
    {"id": "L2", "category": "logistica", "question": "Mostra il fatturato per territorio nel 2014"},
    {"id": "L3", "category": "logistica", "question": "Qual e lo stato con piu ordini clienti?"},
    {"id": "L4", "category": "logistica", "question": "Quanti ordini hanno avuto uno sconto applicato?"},
    # Produzione / Macchinari (proxy via prodotti, make-only, categorie)
    {"id": "P1", "category": "produzione", "question": "Quanti prodotti make only abbiamo?"},
    {"id": "P2", "category": "produzione", "question": "Qual e la categoria con margine piu alto?"},
    {"id": "P3", "category": "produzione", "question": "Qual e il prezzo del prodotto Road-650?"},
    {"id": "P4", "category": "produzione", "question": "Quanti dipendenti ci sono nel reparto Sales?"},
]
...
def _assert_no_hallucinations(body: dict[str, Any]) -> None:
    ...
    assert connectors.issubset(ALLOWED_CONNECTORS), f"Unexpected connector(s): {connectors - ALLOWED_CONNECTORS}"
    assert tables, "Expected grounded lineage tables"
    for table in tables:
        assert table not in SYSTEM_TABLE_MARKERS, f"System table leakage detected: {table}"

    sql_used = str(body.get("sql_used") or "")
    forbidden = ("drop ", "alter ", "delete ", "insert ", "update ", "truncate ")
    assert not any(x in sql_used.lower() for x in forbidden), "Non-deterministic or destructive SQL marker detected"


@pytest.mark.parametrize("case", GOLDEN_QUESTIONS, ids=[f"{c['id']}-{c['category']}" for c in GOLDEN_QUESTIONS])
def test_semantic_ask_golden_questions(
    ...
    assert response.status_code == 200, f"{case['id']} failed with {response.status_code}: {response.text}"

    body = response.json()

    # 1) Intent accuracy
    ontology_intent = (((body.get("provenance") or {}).get("ontology_intent") or {}).get("intent_type"))
    assert isinstance(ontology_intent, str) and ontology_intent.strip(), f"{case['id']}: missing intent"
    assert ontology_intent in allowed_intents, f"{case['id']}: hallucinated intent '{ontology_intent}'"

    # 2) Provenance presence
    lineage = (((body.get("provenance") or {}).get("lineage") or {}))
    assert isinstance(lineage.get("connectors"), list), f"{case['id']}: missing lineage connectors"
    assert isinstance(lineage.get("tables"), list), f"{case['id']}: missing lineage tables"
...
def test_semantic_ask_is_deterministic_for_same_question(
    ...
    first = client.post("/api/semantic/ask", json={"question": question}, headers=auth_headers)
    second = client.post("/api/semantic/ask", json={"question": question}, headers=auth_headers)
...
def test_semantic_ask_rejects_empty_payload(client: TestClient, auth_headers: dict[str, str]) -> None:
...
def test_semantic_ask_handles_out_of_scope_question_with_controlled_error(
    ...
    if response.status_code == 422:
        ...
        assert detail.get("error") in {"SEMANTIC_ONTOLOGY_VIOLATION", "SEMANTIC_SECURITY_VIOLATION"}
