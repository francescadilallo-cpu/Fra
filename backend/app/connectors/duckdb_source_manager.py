    ...
    def _ingest_salesforce(
        ...
        try:
            with SalesforceConnector(
                ...
                # Composite batch describe (25 objects per HTTP call), bounded
                # by FRA_SF_MAX_OBJECTS; priority business objects first, then
                # standard, then custom (__c).
                schema = sf.get_schema_bulk(max_objects=sf_max_objects())
        ...
        # Schema-only fallback when record ingestion is off: one 0-row table
        # per SObject so the KG builder still discovers entity nodes and FK
        # edges from column names.
        n_schema_tables = 0
        if not ingest_records:
            ...
            for sf_obj in schema.objects:
                ...
                # DuckDB resolves `df` by name via replacement scan
                df = pd.DataFrame(cols)  # noqa: F841
                conn.execute(
