...
function SqlBlock({ sql }: { sql: string }) {
  ...
  return (
    <div className="mt-2">
      ...
      {open && (
        <pre className="mt-2 text-xs bg-slate-50 border border-slate-200 rounded-lg p-3 text-slate-700 font-mono overflow-x-auto">
          {sql}
        </pre>
      )}
    </div>
...
function MessageBubble({ message }: { message: Message }) {
  if (message.role === 'user') {
    return (
      <div className="flex justify-end gap-3 items-start">
        ...
        <div className="w-8 h-8 bg-teal-50 border border-teal-200 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
  ...
  if (message.role === 'error') {
    return (
      <div className="flex gap-3 items-start">
        <div className="w-8 h-8 bg-red-50 border border-red-200 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
          <Bot className="w-4 h-4 text-red-500" />
        </div>
        <div className="max-w-[85%] bg-red-50 border border-red-200 rounded-2xl rounded-tl-sm px-4 py-3">
  ...
  return (
    <div className="flex gap-3 items-start">
      <div className="w-8 h-8 bg-white border border-slate-200 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 shadow-sm">
      ...
      <div className="flex-1 max-w-[85%] bg-white border border-slate-200 rounded-2xl rounded-tl-sm px-4 py-3 space-y-3 shadow-sm">
        {/* Interpreted as */}
        <div>
          <span className="text-xs text-slate-400 uppercase tracking-wide">Interpretazione</span>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-8 py-5 border-b border-slate-200 flex-shrink-0 bg-white">
        <h1 className="text-2xl font-semibold text-slate-900">Query AI</h1>
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-8 py-6 space-y-5 bg-slate-50">
        {messages.length === 0 && (
            ...
            <div className="space-y-2 w-full max-w-lg">
              ...
              {SUGGESTED_QUESTIONS.map((q) => (
                <button
                  ...
                  disabled={loading}
                  className="w-full text-left bg-white hover:bg-slate-50 border border-slate-200 hover:border-teal-300 rounded-xl px-4 py-3 text-sm text-slate-600 hover:text-teal-700 transition-all shadow-sm"
                >
                  {q}
                </button>
              ))}
      ...
      <div className="px-8 py-4 border-t border-slate-200 flex-shrink-0 bg-white">
          ...
          <div className="flex gap-2 mb-3 flex-wrap">
            {SUGGESTED_QUESTIONS.map((q) => (
              <button
                ...
                onClick={() => sendMessage(q)}
                disabled={loading}
                className="text-xs bg-white hover:bg-teal-50 border border-slate-200 hover:border-teal-300 rounded-full px-3 py-1 text-slate-500 hover:text-teal-700 transition-all disabled:opacity-50"
              >
                {q}
              </button>
            ))}
        ...
        <div className="flex gap-3 items-end">
          <textarea
            ...
            rows={2}
            disabled={loading}
            className="flex-1 bg-white border border-slate-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-100 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder-slate-400 resize-none outline-none transition-colors disabled:opacity-50"
          />
          <button
        ...
        <p className="text-xs text-slate-400 mt-2">Invio per inviare · Shift+Invio per nuova riga</p>
      </div>
    </div>
  )
}
