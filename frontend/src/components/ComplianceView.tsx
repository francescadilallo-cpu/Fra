...
function GdprRow({ entity }: { entity: EntityCompliance }) {
  ...
  return (
    <tr className={isGray ? 'bg-slate-50/60' : 'bg-white hover:bg-slate-50/40 transition-colors'}>
      <td className="px-3 py-2.5">
        <span className={`text-sm font-medium ${isGray ? 'text-slate-400' : 'text-slate-800'}`}>
          {entity.label}
        </span>
      </td>
      <td className="px-3 py-2.5">
      ...
      <td className="px-3 py-2.5">
        <div className="flex flex-wrap gap-1">
          ...
          {entity.categories.length === 0 && <span className="text-xs text-slate-300">—</span>}
        </div>
      </td>
      <td className="px-3 py-2.5">
        {firstBasis
      ...
      <td className="px-3 py-2.5 text-xs text-slate-600 whitespace-nowrap">{entity.retention}</td>
      <td className="px-3 py-2.5">
...
function AgentCard({ agent }: { agent: AgentCompliance }) {
  return (
    <div className="border border-slate-200 rounded-lg p-3.5 bg-white hover:shadow-sm transition-shadow">
      ...
      <p className="text-xs text-slate-500 leading-relaxed">{agent.rationale}</p>
    </div>
  )
}
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      ...
      <div className="fixed top-4 right-4 z-50 flex flex-col gap-2 pointer-events-none">
        {toasts.map(t => (
          <div key={t.id} className="bg-slate-900 text-white text-sm px-4 py-2.5 rounded-lg shadow-lg flex items-center gap-2 animate-pulse">
      ...
      <div className="mb-6">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500">
              ...
              <span className="font-medium text-slate-700">{workspaceLabel(sector.name)}</span>
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => showToast('ROPA generated — downloading')}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-slate-300 text-sm text-slate-700 hover:bg-slate-50 transition-colors"
            >
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 rounded-full text-xs text-slate-600">
      ...
      <div className="flex gap-4 mb-6">
        ...
        <ScoreBar
          ...
          icon={<Bot size={16} className="text-slate-500" />}
          unassessed={!hasAssessment}
        />
      </div>
        ...
        <div className="col-span-7">
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100">
              ...
              <h2 className="text-sm font-semibold text-slate-800">GDPR Data Map</h2>
              <ChevronRight size={14} className="text-slate-400 ml-auto" />
              <span className="text-xs text-slate-400">{compliance.entities.length} entities</span>
            </div>
                ...
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    {['Entity', 'Personal data', 'Category', 'Lawful basis', 'Retention', 'Residency', 'DPIA'].map(h => (
                      <th key={h} className="px-3 py-2 text-xs font-medium text-slate-500 whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {compliance.entities.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="px-3 py-8 text-center">
                        <p className="text-sm text-slate-400 mb-1">No entities classified yet.</p>
                        {!IS_DEMO_MODE && (
                          <>
        ...
        <div className="col-span-5">
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100">
              ...
              <h2 className="text-sm font-semibold text-slate-800">EU AI Act Risk Register</h2>
              <ChevronRight size={14} className="text-slate-400 ml-auto" />
              <span className="text-xs text-slate-400">{compliance.agents.length} agents</span>
            </div>
            ...
            <div className="px-4 py-3 border-t border-slate-100 bg-slate-50">
              <button
                ...
                className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-white border border-slate-300 text-sm text-slate-700 hover:bg-slate-50 transition-colors"
              >
