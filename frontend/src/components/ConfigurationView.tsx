...
const GOVERNANCE_RULES = [
  { name: 'Audit trail completo di ogni decisione AI',         enabled: true, locked: true,  desc: 'Sempre attivo per compliance EU AI Act Art. 12' },
  { name: 'Human-in-the-loop per decisioni high-risk',          enabled: true, locked: false, desc: 'Approvazione umana per output con confidenza < 80%' },
  { name: 'Data sovereignty: nessun dato esce dall\'EU',        enabled: true, locked: false, desc: 'Solo regioni AWS/Azure europee per processing' },
  { name: 'Right to explanation: ogni output AI ha rationale', enabled: true, locked: false, desc: 'Ragionamento step-by-step disponibile per ogni risposta' },
  { name: 'Federated learning across tenant',                  enabled: false,locked: false, desc: 'Condivisione di pattern senza condividere dati (beta)' },
]
    ...
    <div className={`bg-white border rounded-xl p-4 transition-colors ${connected ? 'border-teal-200' : 'border-slate-200'}`}>
      ...
      <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-slate-100">
        ...
        <span className={`text-xs ${connected ? 'text-teal-700 font-medium' : 'text-slate-400'}`}>
          {connected ? 'Connesso' : 'Disponibile'}
        </span>
      </div>
    </div>
  )
...
const AGENT_BASE = [
  { name: 'Interface Agent',   icon: MessageIcon,   desc: 'Riceve richieste in linguaggio naturale e instrada al giusto specialista',  model: 'claude-sonnet-4', traffic: '1.2k req/giorno' },
  { name: 'Operational Agent', icon: Workflow,      desc: 'Coordina più specialized agent per task multi-step end-to-end',             model: 'claude-sonnet-4', traffic: '340 req/giorno'  },
]

const AGENT_SECTOR: Record<SectorId, { name: string; desc: string; model: string; traffic: string }> = {
  manufacturing: { name: 'Order Specialist',     desc: 'Gestisce il ciclo preventivo→ordine→produzione sull\'ontologia mfg:',  model: 'claude-haiku-4', traffic: '89 req/giorno'  },
  retail:        { name: 'Cart Recovery',        desc: 'Recupera carrelli abbandonati con messaggi personalizzati per cliente', model: 'claude-haiku-4', traffic: '420 req/giorno' },
  healthcare:    { name: 'Care Coordinator',     desc: 'Allinea diagnosi, trattamenti e follow-up con i protocolli clinici',    model: 'claude-haiku-4', traffic: '156 req/giorno' },
  finance:       { name: 'Risk Analyst',         desc: 'Calcola score di rischio aggregando dati da multiple fonti',            model: 'claude-haiku-4', traffic: '210 req/giorno' },
}
...
export default function ConfigurationView() {
  ...
  const agents = [
    ...
    { name: 'Compliance Agent', icon: Shield, desc: 'Verifica ogni decisione contro le policy EU AI Act prima del rilascio', model: 'claude-haiku-4', traffic: '1.2k verifiche/giorno' },
  ]
  ...
  return (
    <div className="p-8 space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Configurazione</h1>
        <p className="text-slate-500 mt-1 text-sm">Personalizza il semantic layer per la tua azienda — connettori, governance, agenti e template di settore.</p>
      </div>
      ...
      <section className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
        <div className="flex items-start justify-between mb-1">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">Pre-built connector per sistemi enterprise — connect once, il semantic layer mappa il resto.</p>
          </div>
          <span className="text-xs bg-teal-50 text-teal-700 font-medium px-2 py-1 rounded-full">
            {connectors.filter(c => c.connected).length} attivi
          </span>
        </div>
      ...
      <section className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
        ...
        <p className="text-xs text-slate-500">Regole automatiche applicate a ogni decisione AI per essere compliant by-design.</p>
      ...
      <section className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
        ...
        <p className="text-xs text-slate-500">Agenti che operano sul semantic layer del settore <strong className="text-teal-700">{SECTORS[sectorId].name}</strong>.</p>
      ...
      <section className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
        ...
        <p className="text-xs text-slate-500">Switch tra template pre-configurati per accelerare il deployment. Ogni template include ontologia, mapping e connettori.</p>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  {isActive && <span className="text-[10px] font-bold bg-teal-600 text-white rounded px-1.5 py-0.5 leading-none">ATTIVO</span>}
                </div>
                ...
                <div className="mt-3 pt-3 border-t border-slate-100 grid grid-cols-3 gap-1.5">
                  <div className="text-center">
                    ...
                    <p className="text-[9px] text-slate-400 uppercase tracking-wide">classi</p>
                  </div>
                  <div className="text-center">
        ...
        <div className="mt-4 flex items-center gap-2 text-xs text-slate-500">
          ...
          <span>Vuoi un template per il tuo settore? Contattaci — sviluppiamo nuovi template in 2-3 settimane.</span>
        </div>
      </section>
    </div>
  )
